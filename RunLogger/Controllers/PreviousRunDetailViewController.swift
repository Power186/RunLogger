...
//        v.delegate = self
    ...
    private func setMapOverlay() {
        ...
        let endLoc = run.locations[run.locations.count - 1]
    ...
    private func centerMap(run: Run) -> MKCoordinateRegion {
        guard let (minLoc, maxLoc) = getMinMaxLocationCoordinate(run: run) else {
    ...
    private func getMinMaxLocationCoordinate(run: Run) -> (min: CLLocationCoordinate2D, max: CLLocationCoordinate2D)? {
